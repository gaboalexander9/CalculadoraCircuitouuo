...
<div id="results" class="container">
    ...
    <div class="pasos">
        ...
        <p>Se resuelve el problema del circuito RL serie con los valores ingresados. Primero se definen los datos de entrada, luego se presentan las fórmulas generales y finalmente se desarrolla el cálculo paso a paso con valores numéricos.</p>
        <h4>1. Datos de entrada (valores ingresados):</h4>
        <ul id="dataList">
            <!-- Los datos se insertarán aquí dinámicamente -->
        </ul>
        <h4>2. Fórmulas generales:</h4>
        <ul>
            <li>Ecuación diferencial: L × (di/dt) + R × i = 0</li>
            <li>Constante de tiempo: τ = L / R</li>
            <li>Corriente: i(t) = I₀ × e<sup>-t / τ</sup></li>
            <li>Voltaje en resistencia: v_R(t) = R × i(t)</li>
            <li>Voltaje en inductancia: v_L(t) = L × (di/dt) = - R × i(t)</li>
            <li>Tiempo para I_objetivo: t_objetivo = - τ × ln(I_objetivo / I₀)</li>
            <li>Error porcentual: | (I_medido - i(t_medido)) / i(t_medido) | × 100 %</li>
        </ul>
        <h4>3. Desarrollo del ejercicio paso a paso:</h4>
        <ol>
            <li><strong>Formular la ecuación diferencial (modelo matemático):</strong><br>
                Aplicando la Ley de Kirchhoff: suma de voltajes = 0.<br>
                v_R + v_L = 0 → R × i + L × (di/dt) = 0.<br>
                Despejando: di/dt = - (R / L) × i.<br>
                Esta es una ecuación diferencial de primer orden, lineal y homogénea.</li>
            <li><strong>Encontrar la solución general:</strong><br>
                La solución para ecuaciones de la forma di/dt + k × i = 0 es i(t) = K × e<sup>-k t</sup>, donde k = R / L.<br>
                Por lo tanto: i(t) = K × e<sup> - (R/L) t</sup>.</li>
            <li><strong>Aplicar condición inicial para encontrar constante K:</strong><br>
                En t = 0, i(0) = I₀.<br>
                i(0) = K × e<sup>0</sup> = K → K = I₀.<br>
                Solución particular: i(t) = I₀ × e<sup> - (R/L) t</sup>.</li>
            <li><strong>Calcular constante de tiempo τ:</strong><br>
                τ = L / R = <span id="tauCalcStep"></span> s (tiempo en que la corriente cae a ≈36,8% de I₀).</li>
            <li><strong>Reescribir i(t) con τ:</strong><br>
                i(t) = I₀ × e<sup> - t / τ</sup> = <span id="i0Step"></span> × e<sup> - t / <span id="tauStep"></span></sup> A.</li>
            <li><strong>Calcular voltajes:</strong><br>
                v_R(t) = R × i(t) = R × I₀ × e<sup> - t / τ</sup> = <span id="vR0Step"></span> × e<sup> - t / τ</sup> V.<br>
                v_L(t) = L × (di/dt) = - R × I₀ × e<sup> - t / τ</sup> = - <span id="vL0Step"></span> × e<sup> - t / τ</sup> V.</li>
            <li><strong>Ejemplo numérico en t = 0:</strong><br>
                i(0) = I₀ × e<sup>0</sup> = <span id="i0Calc"></span> A.<br>
                v_R(0) = R × I₀ = <span id="vR0Calc"></span> V.<br>
                v_L(0) = - R × I₀ = <span id="vL0Calc"></span> V.</li>
            <li><strong>Ejemplo numérico en t = τ:</strong><br>
                i(τ) = I₀ × e<sup>-1</sup> ≈ 0,368 × I₀ = <span id="iTau"></span> A.</li>
            <li><strong>Calcular tiempo para corriente objetivo (si ingresado):</strong><br>
                Partiendo de I_objetivo = I₀ × e<sup> - t_obj / τ</sup>.<br>
                e<sup> - t_obj / τ</sup> = I_objetivo / I₀.<br>
                - t_obj / τ = ln(I_objetivo / I₀).<br>
                t_obj = - τ × ln(I_objetivo / I₀) = <span id="tObj"></span> s.</li>
            <li><strong>Comparación con datos experimentales (si ingresados):</strong><br>
                Calcular i(t_medido) = I₀ × e<sup> - t_medido / τ</sup> = <span id="iMedCalc"></span> A.<br>
                Error porcentual = | (I_medido - i(t_medido)) / i(t_medido) | × 100 % = <span id="errorPerc"></span> %.</li>
        </ol>
...
function calcular() {
    ...
    // Datos para memoria de cálculo
    let dataList = `<li>Resistencia R = ${formato(R)} ohm (Ω).</li>
        <li>Inductancia L = ${formato(L)} henry (H).</li>
        <li>Corriente inicial I₀ = ${formato(I0)} ampere (A).</li>
        <li>Tiempo máximo t_max = ${formato(tmax)} segundos (s).</li>
        <li>Paso de tiempo Δt = ${formato(dt)} segundos (s).</li>`;
    if (!isNaN(Iobj)) dataList += `<li>Corriente objetivo I_objetivo = ${formato(Iobj)} ampere (A).</li>`;
    if (!isNaN(tmed)) dataList += `<li>Tiempo medido t_medido = ${formato(tmed)} segundos (s).</li>
        <li>Corriente medida I_medido = ${formato(Imed)} ampere (A).</li>`;
